...
pub trait YYPMessage: fmt::Debug + Clear + Send + Sync + Sized + 'static {
    ...
    fn write_length_delimited_to(&self, os: &mut CodedOutputStream) -> ProtobufResult<()> {
        ...
        os.write_raw_little_endian32(size)?;  // brianjcj: yyp
        self.write_to_with_cached_sizes(os)?;
        ...
        Self: Sized,
    {
        ...
        let r = YYPMessage::parse_from(&mut is)?;
        is.check_eof()?;
        Ok(r)
    }
    ...
    /// Write the message to bytes vec.
    /// 
    /// > **Note**: You can use [`YYPMessage::parse_from_bytes`]
    /// to do the reverse.
